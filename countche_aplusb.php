...
while ($row = mysql_fetch_array($result)) {
    ...
    if ($length != $length1) {
      ...
      $enter_length = "UPDATE  `major`.`protein_aplusb_combined` SET  `length` =  '$length' WHERE  `protein`.`id` ='$sample_id'";
...
$sum_query = "SELECT SUM(length) FROM protein_aplusb_combined";
...
for ($temp = 0; $temp < 20; $temp++) {
    ...
    $final_query = "UPDATE  `major`.`protensity_aplusb_combined` SET  `alpha_count` =  '$counter_helix[$temp]',
`beta_count` =  '$counter_beta[$temp]',
`coil_count` =  '$counter_coil[$temp]',
`total` =  '$counter_residue[$temp]',
`p_alpha` =  '$p_helix[$temp]',
`p_beta` =  '$p_beta[$temp]',
`p_coil` =  '$p_coil[$temp]' WHERE  `protensity_aplusb_combined`.`r_name` ='$char1[$temp]'";
    mysql_query($final_query);
}


$display= "SELECT * FROM  `protensity_aplusb_combined`";
